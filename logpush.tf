resource "cloudflare_logpush_job" "fairlane_logpush" {
  ...
  destination_conf = "https://api${var.development ? ".dev" : ""}.botmanagement.com/api/logs/cloudflare/worker?header_X-Auth-Customer-Id=${var.fairlane_customer_id}&header_X-Auth-Token=${var.fairlane_api_token}&header_Content-Type=application%2Fjson"
